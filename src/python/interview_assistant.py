...
FALLBACK_TEMPLATES = [
    {
        "question": "Describe a challenging project you worked on as a {role}.",
        "follow_up": "What technical trade-offs did you make?",
        "difficulty": "Mid-Level",
    },
    {
        "question": "How do you stay updated with the latest trends in {role}?",
        "follow_up": "Can you give a real example?",
        "difficulty": "Junior",
    },
    {
        "question": "Tell me about a conflict you had with a team member.",
...
def generate_question(role="Software Engineer", level="Mid-Level", history=None):
    ...
    level_desc = "Focus on fundamentals."
    if "Senior" in level:
        level_desc = "Focus on system design and leadership."
    elif "Junior" in level:
        level_desc = "Focus on core concepts."

    history_str = ", ".join(h.get("text", "") for h in history) or "None"
    ...
    try:
        ...
        return {
            "question": fallback["question"].format(role=role),
...
def analyze_answer(transcript: str, question_context="General Interview"):
    if not transcript or len(transcript) < 5:
        return {
            "strengths": ["N/A"],
    ...
    try:
        ...
        data.setdefault("clarity_score", 70)
        data.setdefault("confidence_estimate", 75)
        data.setdefault("filler_words_count", {})
